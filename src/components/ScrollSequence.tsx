...
export default function ScrollSequence() {
    ...
    const drawFrame = useCallback((index: number) => {
        ...
        // Clear and draw with "cover" fit (fills entire viewport)
    ...
    function useOverlayOpacity(start: number, end: number) {
        const fadeIn = start + 0.03;
        const fadeOut = end - 0.03;
        return useTransform(scrollYProgress, [start, fadeIn, fadeOut, end], [0, 1, 1, 0]);
    }

    function useOverlayY(start: number, end: number) {
        const fadeIn = start + 0.03;
        const fadeOut = end - 0.03;
        return useTransform(scrollYProgress, [start, fadeIn, fadeOut, end], [40, 0, 0, -40]);
    }
...
function TextOverlay({
    ...
    overlay: Overlay;
    useOverlayOpacity: (s: number, e: number) => ReturnType<typeof useTransform>;
    useOverlayY: (s: number, e: number) => ReturnType<typeof useTransform>;
}) {
    const opacity = useOverlayOpacity(overlay.start, overlay.end);
    const y = useOverlayY(overlay.start, overlay.end);
